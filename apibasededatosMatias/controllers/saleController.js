...
const procesarVenta = async (req, res) => {
    ...
    try {
        ...
        for (const pago of pagos_mixtos) {
            await client.query(`INSERT INTO ventas_pagos (id_venta, id_metodo_pago, monto_pagado) VALUES ($1, $2, $3)`, [id_venta, pago.id_metodo_pago, pago.monto_pagado]);
        }

        for (const item of detalle_productos) {
            await client.query(`INSERT INTO ventas_detalle (id_venta, id_producto, cantidad, precio_unitario, subtotal) VALUES ($1, $2, $3, $4, $5)`, [id_venta, item.id_producto, item.cantidad, item.precio_unitario, item.subtotal]);
            ...
            await client.query(`INSERT INTO kardex (id_producto, id_sucursal, tipo_movimiento, cantidad, stock_posterior, fecha, id_usuario, referencia_id) VALUES ($1, $2, 'VENTA', $3, $4, NOW(), $5, $6)`, [item.id_producto, id_sucursal, -item.cantidad, resStock.rows[0].stock_actual, id_usuario, id_venta]);
        }
...
// NUEVA API: Obtener el historial de tickets de venta
const getHistorialVentas = async (req, res) => {
    try {
        const query = `
            ...
            ORDER BY vc.fecha DESC LIMIT 100
        `;
